...
fetchPurchases :: MVar Connection -> MVar User -> Int -> IO ()
fetchPurchases mConn mUser limit = do
  printHeader
  printPurchases 0
  where
    printPurchases curOffset = fetchPurchases' mConn mUser curOffset limit >>= \case
      []        -> pure ()
      purchases -> do
        prettyPrintPurchases purchases
